...
class PredictiveController(OSKenApp):
    """
    Prediction-only controller baseline
    """
    ...
    def __init__(self, *args, **kwargs):
        super(PredictiveController, self).__init__(*args, **kwargs)

        # Run ID and output directory
        self._run_id = generate_run_id()
        self._run_dir = setup_run_directory("predictive", self._run_id)
        ...
        # CSV loggers (no EventCSVLogger -- no reactive layer)
        ...
        self.current_path = Path.PATH_A  # Default

        # Flow cache to prevent duplicate installations
        self._installed_flows = set()

        # Stats polling
        self._stats_interval = 1.0

        # Flow modification counter
        self.flow_mod_count = 0

        self.logger.info("[START] Prediction-only controller initialized (no reactive layer)")
    ...
    def _configure_predictive_layer(self):
        """Configure predictive layer after topology discovery."""
    ...
    def switch_features_handler(self, ev):
        ...
        # Start when all switches are connected
        if len(self.datapaths) == 4:
            self._start_predictive()
    ...
    def _handle_ipv4(self, datapath, dpid, in_port, eth, ip_pkt, msg):
        """Handle IPv4 packets with predictive path selection."""
        parser = datapath.ofproto_parser

        # Learn source MAC
        self.mac_port_map[dpid][eth.src] = in_port

        # Path selection only at ingress switch
        if dpid == self.DPID_S1 and ip_pkt.src == self.H1_IP and ip_pkt.dst == self.H2_IP:
            out_port = self.S1_PORT_S2 if self.current_path == Path.PATH_A else self.S1_PORT_S3
            actions = [parser.OFPActionOutput(out_port)]
            self._send_packet(datapath, msg, actions)
            return

        # For reverse traffic use shortest path
        out_port = self._get_forwarding_port(dpid, ip_pkt)
        if out_port:
            actions = [parser.OFPActionOutput(out_port)]
    ...
    def _get_forwarding_port(self, dpid, ip_pkt) -> int:
    ...
    def _delete_flows_by_match(self, datapath, match):
    ...
    def _on_planning_decision(self, decision: PlanningDecision):
        ...
        # Update baseline path if changed
        if decision.selected_path != self.current_path:
    ...
    def _start_predictive(self):
        """Start predictive layer after all switches connected."""
        self.logger.info("[START] All switches connected - starting predictive layer")

        # Configure predictive layer with topology information
        self._configure_predictive_layer()
